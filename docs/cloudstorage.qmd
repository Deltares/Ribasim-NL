...
::: {.callout-warning}
This page documents the legacy The Good Cloud storage interface. It is not the main storage method for current workflows. Use MinIO through DVC instead; see the [DVC workflow](dev/dvc.qmd) and [developer setup](dev/index.qmd#sec-environment-variables).
:::

Ribasim-NL has historically used this cloud storage: https://deltares.thegood.cloud/.
...
This is a legacy storage interface. Most current workflows use MinIO and require the AWS credentials documented in the [developer setup](/dev/index.qmd#sec-environment-variables). The variables specific to legacy `CloudStorage` code are:
