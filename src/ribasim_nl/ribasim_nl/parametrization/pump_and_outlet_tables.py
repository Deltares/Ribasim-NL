    ...
    for row in defaults_df.itertuples():
        ...
        if sub_mask.any():
            ...
            if not pd.isna(row.flow_rate_mm_per_day):
                unit_conversion = float(row.flow_rate_mm_per_day) / 1000 / 86400
        ...
        if sub_mask.any():
            ...
            upstream_level_offset = float(row.upstream_level_offset)
        ...
        if sub_mask.any():
            ...
            downstream_level_offset = float(row.downstream_level_offset)
