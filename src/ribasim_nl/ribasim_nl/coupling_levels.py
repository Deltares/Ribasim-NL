...
def flow_demand_targets(
    ...
    for target_node_id, rows in control_link_df.groupby("to_node_id"):
        ...
        if positive_flow_demand_target:
            positive_target_node_ids.add(int(target_node_id))
        controlled_node_ids.add(int(target_node_id))
        target_info[int(target_node_id)] = {
...
def connector_report_record(
    ...
    upstream_node_type = context.node_type_by_id.get(upstream_id)
    downstream_node_type = context.node_type_by_id.get(downstream_id)
    upstream_authority = context.basin_authority_by_id.get(upstream_id)
    downstream_authority = context.basin_authority_by_id.get(downstream_id)

    rws_to_model_link = (
        upstream_authority == RWS_AUTHORITY and pd.notna(downstream_authority) and downstream_authority != RWS_AUTHORITY
    )
    min_upstream_is_coupling_link = rws_to_model_link or (
        pd.notna(row.meta_waterbeheerder)
        and pd.notna(upstream_authority)
    ...
    direct_downstream_targets = (
        downstream_targets(int(max_basin_id), context.outgoing_flow_links, context.node_type_by_id)
    ...
    direct_downstream_control_node_ids = [
        ...
        if error is None and context.node_type_by_id.get(target_node_id) in CONTROL_NODE_TYPES
    ]
    ...
    limburg_rws_flow_demand_min_upstream = (
        ...
        and (
            (pd.notna(row.meta_waterbeheerder) and str(row.meta_waterbeheerder) == LIMBURG_AUTHORITY)
            or (pd.notna(downstream_authority) and str(downstream_authority) == LIMBURG_AUTHORITY)
        )
    )
    ...
    return {
        **row_dict,
        ...
        "upstream_basin_name": context.basin_name_by_id.get(upstream_id),
        ...
        "downstream_basin_name": context.basin_name_by_id.get(downstream_id),
        ...
        "max_downstream_level_basin_authority": context.basin_authority_by_id.get(max_basin_id),
...
def rws_leak_records(context: CouplingReportContext) -> list[dict[str, object]]:
    ...
    for node_id, rows in context.static_df.groupby("node_id"):
        upstream_id, upstream_link_id, _ = first_non_junction(
            int(node_id), context.incoming_flow_links, context.node_type_by_id
        )
        downstream_id, downstream_link_id, _ = first_non_junction(
            int(node_id), context.outgoing_flow_links, context.node_type_by_id
        )
        upstream_authority = context.basin_authority_by_id.get(upstream_id)
        downstream_authority = context.basin_authority_by_id.get(downstream_id)
        if not (
            context.node_type_by_id.get(upstream_id) == "Basin"
            and context.node_type_by_id.get(downstream_id) == "Basin"
            and upstream_authority == RWS_AUTHORITY
            and pd.notna(downstream_authority)
            and downstream_authority != RWS_AUTHORITY
        ):
        ...
        node_row = context.node_by_id[int(node_id)]
        records.append(
            {
                "node_id": int(node_id),
                "node_type": node_row.get("node_type"),
                "functie": context.functions[int(node_id)],
                "meta_waterbeheerder": node_row.get("meta_waterbeheerder"),
                "meta_code_waterbeheerder": node_row.get("meta_code_waterbeheerder"),
                "meta_node_id_waterbeheerder": node_row.get("meta_node_id_waterbeheerder"),
                "name": node_row.get("name"),
