...
def reset_index(model: Model, node_start=1):
    ...
    if not (
        (node_start == node_id_min) and (expected_length == len(model.node_table().df))
    ):
        ...
        # renumber edges
        model.edge.df.loc[:, ["from_node_id"]] = model.edge.df["from_node_id"].apply(
            lambda x: index.loc[x]
        )

        model.edge.df.loc[:, ["to_node_id"]] = model.edge.df["to_node_id"].apply(
            lambda x: index.loc[x]
        )
