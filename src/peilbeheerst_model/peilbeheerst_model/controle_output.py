...
class Control:
    ...
    def initial_final_level(self, control_dict):
        basin_level = self.df_basin.sort_values(by=['time', 'node_id'], ascending = True).copy() #sort all node_id's based on time
        initial_final_level_df = gpd.GeoDataFrame()
        
        ini_basin_level = basin_level.drop_duplicates(subset='node_id', keep='first').reset_index(drop=True) #pick the FIRST timestamp, which are the initial water levels
        initial_final_level_df['node_id'] = ini_basin_level['node_id']
        initial_final_level_df['initial_level'] = ini_basin_level['level']
        
        final_basin_level = basin_level.drop_duplicates(subset='node_id', keep='last').reset_index(drop=True) #pick the LAST timestamp, which are the final water levels
        initial_final_level_df['final_level'] = final_basin_level['level']
        initial_final_level_df['difference_level'] = initial_final_level_df['initial_level'] - initial_final_level_df['final_level']
        
        initial_final_level_df['final_level_within_target'] = True #final level within target level (deviate max 20 cm from streefpeil) is default True ...
        initial_final_level_df.loc[(initial_final_level_df['difference_level'] > 0.2) | (initial_final_level_df['difference_level'] < -0.2), 'final_level_within_target'] = False # ... but set to False if the criteria is not met

        #retrieve the geometries
        initial_final_level_df['geometry'] = pd.merge(left = initial_final_level_df,
    ...
    def min_max_level(self, control_dict):
        basin_level = self.df_basin.sort_values(by=['level', 'node_id'], ascending = True).copy() #sort all node_id's based on the level

        min_basin_level_df = basin_level.drop_duplicates(subset='node_id', keep='first').reset_index(drop=True) #pick the FIRST sample, which are the minimum water levels
        min_max_basin_level_df = min_basin_level_df[['node_id', 'level', 'time']]         
        min_max_basin_level_df = min_max_basin_level_df.rename(columns={'level': 'min_level',
                                                                        'time': 'min_level_time'})
        
        max_basin_level_df = basin_level.drop_duplicates(subset='node_id', keep='last').reset_index(drop=True) #pick the LAST sample, which are the maximum water levels
        ...
        #retrieve the geometries
        min_max_basin_level_df['geometry'] = pd.merge(left = min_max_basin_level_df,
                                                      right = self.model.basin.node.df,
                                                      on = 'node_id',
                                                      suffixes = ('', 'model_'))['geometry']
    ...
    def error(self, control_dict):
        ...
        basin_error = basin_error.sort_values(by=['node_id', 'relative_error'], ascending = True) #sort all node_id's based on the level

        max_error = self.df_basin.drop_duplicates(subset='node_id', keep='last').reset_index(drop=True) #pick the LAST sample, which are the largest errors in the water balance
        ...
        #retrieve the geometries
        error_gdf['geometry'] = pd.merge(left = error_gdf,
                                                      right = self.model.basin.node.df,
                                                      on = 'node_id',
                                                      suffixes = ('', 'model_'))['geometry']
    ...
    def stationary(self, control_dict):
        ...
        # Retrieve the geometries
        stationary_gdf['geometry'] = pd.merge(
            left=stationary_gdf,
            right=self.model.basin.node.df,
            on='node_id',
            suffixes=('', 'model_')
    ...
    def store_data(self, data, output_path):
        ...
        shutil.copy(src=output_controle_qlr_path, 
                    dst=os.path.join(self.work_dir, 'results', 'output_controle.qlr'))
