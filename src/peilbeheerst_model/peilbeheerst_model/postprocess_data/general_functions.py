...
def overlapping_peilgebieden(waterschap_peilgebieden):
    ...
    for index, row in peilgebied.iterrows():
        ...
        overlaps = other_polygons[other_polygons.geometry.overlaps(current_polygon.geometry.iloc[0])] #check for overlaps with other polygons

        if not overlaps.empty:
            #calculate the percentage of overlap, and add this to the gdf including the overlapping indexes
            current_overlap_percentage = overlaps.geometry.intersection(current_polygon.geometry.iloc[0]).area / current_polygon.geometry.iloc[0].area * 100

            overlaps['overlap_percentage'], overlaps['source_globalid'] = pd.NA, pd.NA #create columns

            #fill columns
            overlaps['overlap_percentage'] = current_overlap_percentage #multiple peilgebieden will be added to the temporal gdf if there are multiple overlapping polygons
            overlaps['source_globalid'] = current_polygon['globalid'].values[0] #add the global id of the current polygon. 
...
def plot_overlapping_peilgebieden(peilgebied, overlapping_polygons, minimum_percentage):
    ...
    overlap_subset.plot(ax = ax, cmap = 'coolwarm', column = overlap_subset.overlap_percentage, label = 'Percentage of overlap', legend = True)
...
def burn_in_peilgebieden(base_layer, overlay_layer, plot=True):
    ...
    base_layer_without_overlapping = gpd.overlay(base_layer, overlay_layer, how='symmetric_difference', keep_geom_type=False) ##remove the difference between pg_praktijk and pg_afwijking
    ...
    if 'waterhoogte_1' in base_layer_without_overlapping.keys(): #sometimes a waterhoogte is present in the peilgebieden. Manage this.
        base_layer_without_overlapping.rename(columns={'code_1': 'code',
                                                       'nen3610id_1': 'nen3610id',
                                                       'globalid_1': 'globalid',
                                                       'waterhoogte_1': 'waterhoogte'}, inplace = True)
        base_layer_without_overlapping.drop(columns = ['code_2', 'nen3610id_2', 'globalid_2', 'waterhoogte_2'], inplace=True)
        
    else:
        base_layer_without_overlapping.rename(columns={'code_1': 'code',
                                               'nen3610id_1': 'nen3610id',
                                               'globalid_1': 'globalid'}, inplace = True)
