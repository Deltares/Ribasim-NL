...
class AddStorageBasin:
    ...
    def add_basin_nodes_with_manning_resistance(self):
        ...
        for index, row in self.model.basin.node.df.iterrows():
            ...
            meta_categorie = state_df.loc[state_df['node_id'] == node_id, 'meta_categorie'].values
            ...
            if self.include_hoofdwater:
                if 'bergend' in meta_categorie or ('hoofdwater' not in meta_categorie and 'doorgaand' not in meta_categorie):
                    continue
            else:
                        ...
                        if sub_key == 'time' or sub_key == 'subgrid':
                            ...
                            if 'geometry' in last_row:
                                ...
                                if sub_key == 'node':
                                    ...
                                    last_row['geometry'] = original_geometry if original_geometry is not None else geometry
    ...
    def connect_nodes(self, new_basin_node_id, manning_node_id, original_node_id):
        ...
        try:
            ...
            logging.info(f"Connected new Basin Node ID: {new_basin_node_id} to original Basin Node ID: {original_node_id} via Manning Resistance Node ID: {manning_node_id}")
        except Exception as e:
