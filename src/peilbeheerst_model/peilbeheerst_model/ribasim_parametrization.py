...
def insert_standard_profile(ribasim_model, regular_percentage = 10, boezem_percentage = 90, depth_profile = 2):
    ...
    node_id_boezem = ribasim_model.basin.state.df.loc[ribasim_model.basin.state.df.meta_categorie == 'hoofdwater', 'node_id'].values
...
def FlowBoundaries_to_LevelBoundaries(ribasim_model, default_level = 0):
    ...
    ribasim_model.tabulated_rating_curve.node.df = pd.concat([ribasim_model.tabulated_rating_curve.node.df,
                                                              nodes_TRC_FlowBoundary]).reset_index(drop=True)
    ...
    nodes_FlowBoundary['geometry'] = nodes_FlowBoundary.geometry.translate(xoff=-1, yoff=-1) #move the points 1 meter to the lower left (diagonally)
    ...
    ribasim_model.level_boundary.node.df = pd.concat([ribasim_model.level_boundary.node.df,
                                                      nodes_LevelBoundary[['node_id', 'node_type', 'geometry']]]).reset_index(drop=True)
    #supplement the LB.static table
    ribasim_model.level_boundary.static.df = pd.concat([ribasim_model.level_boundary.static.df,
                                                      nodes_LevelBoundary[['node_id', 'level']]]).reset_index(drop=True)

    #the nodes have been created. Now add the edges
    edges_LB = pd.DataFrame()
    edges_LB['from_node_id'] = nodes_LevelBoundary['node_id'] #as these nodes were initially FlowBoundaries, they always flow into the model, not out. Thus, is always the starting point (=from_node_id)
...
def add_outlets(ribasim_model, delta_crest_level = 0.10):
    ...
    TRC_naar_OL = ribasim_model.tabulated_rating_curve.static.df.loc[ribasim_model.tabulated_rating_curve.static.df.meta_type_verbinding == 'Inlaat']
    ...
    target_level = pd.merge(left = TRC_naar_OL,
                            right = ribasim_model.edge.df,
                            left_on = 'node_id',
                            right_on = 'to_node_id',
                            how = 'left')
    ...
    target_level = pd.merge(left = target_level,
                            right = ribasim_model.basin.state.df,
    ...
    target_level.rename(columns={'level': 'min_crest_level',
                                 'node_id_x': 'node_id'}, inplace=True)

    outlet = target_level.copy(deep=True)
    outlet['min_crest_level'] -= delta_crest_level # the peil of the boezem is allowed to lower with this much before no water will flow through the outlet, to prevent 
    get_outlet_geometries = ribasim_model.tabulated_rating_curve.node.df.loc[ribasim_model.tabulated_rating_curve.node.df.node_id.isin(outlet.node_id.values)]
    outlet = pd.merge(left = outlet,
                      right = get_outlet_geometries[['node_id', 'geometry']],
                      on = 'node_id')
    ...
    ribasim_model.tabulated_rating_curve.node = ribasim_model.tabulated_rating_curve.node.df.loc[~ribasim_model.tabulated_rating_curve.node.df.node_id.isin(outlet.node_id)].reset_index(drop=True)
    ribasim_model.tabulated_rating_curve.static = ribasim_model.tabulated_rating_curve.static.df.loc[~ribasim_model.tabulated_rating_curve.static.df.node_id.isin(outlet.node_id)].reset_index(drop=True)
...
def set_tabulated_rating_curves(ribasim_model, level_increase=1.0, flow_rate=4):
    ...
    df_tab =  pd.merge(ribasim_model.basin.state.df,df_edge_tab, left_on='node_id', right_on='from_node_id', how='inner')
    df_tab = df_tab[['to_node_id', 'active', 'level_x', 'flow_rate', 'control_state']]
...
def write_ribasim_model_GoodCloud(ribasim_model, path_ribasim_toml, waterschap, modeltype = 'boezemmodel', include_results = True):
    ...
    with open(path_goodcloud_password, 'r') as file:
        password = file.read()
...
def iterate_TRC(ribasim_param, allowed_tolerance, max_iter, expected_difference, max_adjustment, cmd, output_dir, path_ribasim_toml):
            ...
            for basin_node, trc_group in df_trc.groupby("from_node_id"):
                ...
                if abs(basin_diff) < allowed_tolerance:
                    ...
                    for row in trc_group.itertuples():
                        ...
                        if (df_trc_static.at[idx_lastpoint, "level"] + trc_adjust) < df_trc_static.at[idx_firstpoint, "level"]:
                            trc_adjust = df_trc_static.at[idx_lastpoint, "level"] - df_trc_static.at[idx_firstpoint, "level"] - 1e-2
