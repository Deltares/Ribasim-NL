...
def generate_ribasim_nodes_static(
    ...
    ribasim_nodes_static = gpd.GeoDataFrame(data=nodes, geometry="geometry", crs=split_nodes.crs)
...
def generate_ribasim_basins(
    ...
    ribasim_basins = ribasim.Basin(
        profile=basin_profile, time=basin_time, state=basin_state, subgrid=basin_subgrid, area=basin_areas
    )
...
def generate_ribasim_model(
    ...
    ribasim_level_boundaries = generate_ribasim_level_boundaries(
        level_boundary_static=tables["level_boundary_static"], level_boundary_time=tables["level_boundary_time"]
    )

    ribasim_flow_boundaries = generate_ribasim_flow_boundaries(
        flow_boundary_static=tables["flow_boundary_static"], flow_boundary_time=tables["flow_boundary_time"]
    )

    ribasim_pumps = generate_ribasim_pumps(pump_static=tables["pump_static"])

    ribasim_outlets = generate_ribasim_outlets(outlet_static=tables["outlet_static"])
