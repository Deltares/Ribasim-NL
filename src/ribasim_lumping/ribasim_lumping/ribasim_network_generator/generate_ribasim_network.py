...
def split_graph_based_on_split_nodes(
    ...
    # update split nodes gdf with new node no
    split_nodes.loc[split_nodes_nodes.index, "graph_node_no"] = pd.Series(
        new_graph_node_no, index=split_nodes_nodes.index, dtype=object
    )
...
def add_basin_code_from_network_to_nodes_and_links(
    ...
    split_nodes.loc[ix, "graph_node_no"] = pd.Series(
        [(x,) for x in split_nodes.loc[ix, "node_no"]], index=ix, dtype=object
    )
    orig_node_indexer = {
...
def create_basin_areas_based_on_drainage_areas(
    ...
    def get_area_code_for_lateral(laterals, areas):
        ...
        if len(selected_areas) == 0:
            ...
            return selected_areas["code"].values[0]
    ...
    if laterals is not None:
        ...
        areas["basin"] = areas.apply(lambda x: get_basin_code_from_lateral(x["code"], laterals_join), axis=1)
...
def create_basins_based_on_subgraphs_and_nodes(
    ...
    if ~basins2.empty:
        ...
        basins2b = (
            basins_temp2.groupby("basin")
            .last()
            .reset_index()[["basin", "node_no"]]
            .rename(columns={"node_no": "node_no2"})
        )
        ...
        basins2s = gpd.GeoDataFrame(basins2s, geometry=[v.centroid for v in basins2s.geometry.values], crs=basins.crs)
...
def create_basin_connections(
    ...
    conn_ds = conn_ds.merge(basins[["basin", "geometry"]], left_on="basin", right_on="basin").rename(
    ...
    conn_us = conn_us.merge(basins[["basin", "geometry"]], left_on="basin", right_on="basin").rename(
        columns={"geometry": "geom_basin"}
    )
    ...
    if option_links_hydroobjects:
        ...
        _basin_connections = _basin_connections.merge(
            basins[["basin", "node_no"]].rename(columns={"node_no": "basin_node_no"}), how="left", on="basin"
        )
        # get shortest paths
        _basin_connections["paths"] = [
            nx.dijkstra_path(graph, sn, bn) if c == "split_node_to_basin" else nx.dijkstra_path(graph, bn, sn)
            for sn, bn, c in zip(
...
def create_boundary_connections(
    ...
    boundaries_conn = boundaries[["boundary_id", "boundary_type", "geometry"]].sjoin_nearest(
        nodes[["node_no", "geometry", "basin"]]
    )

    boundaries_conn = boundaries_conn.drop(columns=["index_right"]).rename(columns={"node_no": "boundary_node_no"})
    boundaries_conn.crs = nodes.crs
    boundaries_conn = boundaries_conn.rename(columns={"geometry": "geom_boundary"}).merge(
        basins[["basin", "geometry"]].rename(columns={"geometry": "geom_basin"}), how="left", on="basin"
    )
    ...
    if not waterlevelbnd_conn.empty:
        ...
        boundaries_conn = gpd.GeoDataFrame(
            pd.concat([dischargebnd_conn_in, waterlevelbnd_conns]).reset_index(drop=True),
            geometry="geometry",
            crs=28992,
        )
    else:
...
def remove_basins_from_boundary(
    boun_conn: gpd.GeoDataFrame,
    ...
    boun_conn_basins_out = boun_conn_basins[
        (boun_conn_basins["split_node"] != -1) & (boun_conn_basins["connection"] == "basin_to_boundary")
    ]
    if not boun_conn_basins_out.empty:
        ...
        boun_conn_basins_out["geometry"] = boun_conn_basins_out.apply(
            lambda x: LineString([x["geom_split_node"], x["geom_boundary"]]),
            axis=1,
        )
        nodes.loc[nodes.basin.isin(boun_conn_basins_out.basin.values), "basin"] = -1

    boun_conn_basins_in = boun_conn_basins[
        (boun_conn_basins["split_node"] != -1) & (boun_conn_basins["connection"] == "boundary_to_basin")
    ]
    if not boun_conn_basins_in.empty:
        ...
        boun_conn_basins_in["geometry"] = boun_conn_basins_in.apply(
            lambda x: LineString([x["geom_boundary"], x["geom_split_node"]]),
            axis=1,
        )
...
def regenerate_node_ids(
    ...
    basin_connections["from_node_id"] = basin_connections.apply(
        lambda x: x["basin_node_id"] if x["connection"] == "basin_to_split_node" else x["split_node_node_id"], axis=1
    )
    basin_connections["to_node_id"] = basin_connections.apply(
        lambda x: x["basin_node_id"] if x["connection"] == "split_node_to_basin" else x["split_node_node_id"], axis=1
    )
    ...
    split_nodes = gpd.GeoDataFrame(
        split_nodes.merge(
            connections.set_index("to_node_id"), how="left", left_on="split_node_node_id", right_index=True
        ),
        geometry="geometry",
        crs=split_nodes.crs,
    )
    split_nodes = gpd.GeoDataFrame(
        split_nodes.merge(
            connections.set_index("from_node_id"), how="left", left_on="split_node_node_id", right_index=True
        ),
        geometry="geometry",
        crs=split_nodes.crs,
    )
    ...
    boundaries = boundaries.drop_duplicates(keep="first")
    split_nodes = split_nodes.drop_duplicates(keep="first")
...
def remove_isolated_basins_and_update_administration(
    ...
    isolated_basin_to_basin = {
        ib: basin_area_to_basin[basin_to_basin_area[ib]] for ib in isolated_basins if ib in basin_to_basin_area.keys()
    }
    isolated_basin_to_basin = {
        k: v[np.isin(v, basins.loc[np.isin(basins["basin"].values, connected_basins)]["basin"])]
        for k, v in isolated_basin_to_basin.items()
    }
...
def generate_ribasim_network_using_split_nodes(
    ...
    basin_areas = remove_holes_from_basin_areas(basin_areas=basin_areas, min_area=remove_holes_min_area)
