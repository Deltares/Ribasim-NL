...
def get_ribasim_basin_data_from_model(
    ribasim_model: ribasim.Model, ribasim_results: RibasimModelResults, basin_no: int
):
    basin_profile = ribasim_model.basin.profile.df.groupby("node_id").get_group(basin_no).drop(columns=["node_id"])
    model_basin = ribasim_results.basin.copy()
    model_control = ribasim_results.control.copy()
    model_flow = ribasim_results.flow.copy()

    basin = (
        model_basin.reset_index()[model_basin.reset_index().node_id == basin_no]
...
def plot_results_basin_ribasim_model(
    ...
    if basin_results is None:
        basin_results = get_ribasim_basin_data_from_model(
            ribasim_model=ribasim_model, ribasim_results=ribasim_results, basin_no=basin_no
        )
    ...
    if complete:
        ...
        ax4.hlines(
            y=basin_results.basin_profile.level.min(),
            xmin=0.0,
            xmax=basin_results.outflow.flow_rate.max() * 1.1,
            linestyle="-",
            color="black",
        )
