...
class RibasimLumpingNetwork(BaseModel):
    ...
    def add_basis_network(
        ...
        if source_type == "dhydro":
            ...
            if results is not None:
                        ...
                        if old_gdf is None:
                            ...
                            return gpd.GeoDataFrame(
                                old_gdf.drop(columns=["geometry", set_name], errors="ignore").merge(
                                    new_gdf[[set_name, "geometry"]], how="outer", left_index=True, right_index=True
                                ),
                                geometry="geometry",
                                crs=old_gdf.crs,
                            )
                ...
                self.uniweirs_gdf = combine_old_gdf_with_new_gdf_using_setname(
                    self.uniweirs_gdf, uniweirs_gdf, set_name
                )
                self.orifices_gdf = combine_old_gdf_with_new_gdf_using_setname(
                    self.orifices_gdf, orifices_gdf, set_name
                )
                self.culverts_gdf = combine_old_gdf_with_new_gdf_using_setname(
                    self.culverts_gdf, culverts_gdf, set_name
                )
            ...
            results = add_hydamo_basis_network(
                hydamo_network_file=hydamo_network_file, hydamo_split_network_dx=hydamo_split_network_dx
            )
            if results is not None:
                (
                    self.branches_gdf,
    ...
    def read_areas(
        ...
        self.areas_gdf = read_geom_file(
            filepath=areas_file_path, layer_name=layer_name, crs=crs, explode_geoms=False, code_column=areas_code_column
        )
    ...
    def read_drainage_areas(
        ...
        no_drainage_areas = len(self.drainage_areas_gdf) if self.drainage_areas_gdf is not None else 0
    ...
    def read_boundaries(
        ...
        self.boundaries_gdf = read_geom_file(
            filepath=boundary_file_path, layer_name=layer_name, crs=crs, remove_z_dim=True
        )
        ...
        self.boundaries_gdf = self.boundaries_gdf.loc[
            [i not in boundaries_not_on_network.index for i in self.boundaries_gdf.index]
        ]
    ...
    def export_or_update_all_ribasim_structures_specs(self, structure_specs_dir_path: Path):
        ...
        for structure_type in ["pump", "weir", "orifice", "culvert"]:
            ...
            self.export_or_update_ribasim_structure_specs(
                structure_type=structure_type, structure_specs_path=structure_specs_path
            )
    ...
    def export_or_update_ribasim_structure_specs(self, structure_type: str, structure_specs_path: Path):
        ...
        control_headers = [
            ...
            "upstream_setpoint",
            "upstream_lowerlimit",
            "downstream_upperlimit",
            "downstream_setpoint",
            "downstream_lowerlimit",
        ]
        sets_columns = {
            "pump": [
        ...
        if not structure_specs_path.exists():
            print(
                f" x ribasim_input_{structure_type} ('{structure_specs_path}') DOES NOT exist. network.{structure_type}s_gdf will be EXPORTED."
            )
        ...
        print(
            f" x ribasim_input_{structure_type} ('{structure_specs_path}') DOES exist. network.{structure_type}s_gdf will be UPDATED."
        )
    ...
    def read_split_nodes(
        ...
        split_nodes = read_geom_file(filepath=split_nodes_file_path, layer_name=layer_name, crs=crs, remove_z_dim=True)
        ...
        hydamo_objects.object_type = pd.Categorical(hydamo_objects.object_type, categories=gdf_names, ordered=True)
        hydamo_objects = hydamo_objects.sort_values("object_type")
        hydamo_objects_buffer = hydamo_objects[["object_type", "code", "geometry"]].rename(
            columns={"code": "split_node_id"}
        )
        hydamo_objects_buffer.geometry = hydamo_objects_buffer.geometry.buffer(0.001)

        new_split_nodes = gpd.sjoin(
            new_split_nodes[["object_type", "object_function", "geometry"]].rename(
                columns={"object_type": "split_node_object_type"}
            ),
    ...
    def read_areas_laterals_timeseries(
        self, areas_laterals_path: Path, sep: str = ",", index_col: int = 0, dayfirst=False
    ):
        """Read timeseries data laterals"""
        self.laterals_areas_data = pd.read_csv(
            areas_laterals_path, index_col=index_col, sep=sep, parse_dates=True, dayfirst=dayfirst
        )
    ...
    def read_boundaries_timeseries_data(self, boundaries_timeseries_path: Path, skiprows=0, sep=",", index_col=0):
        ...
        boundary_csv_data = pd.read_csv(
            boundaries_timeseries_path, sep=sep, skiprows=skiprows, index_col=index_col, parse_dates=True
        )
    ...
    def generate_ribasim_model_complete(
        ...
        self.write_ribasim_model(starttime=starttime, endtime=endtime)
        return ribasim_model
