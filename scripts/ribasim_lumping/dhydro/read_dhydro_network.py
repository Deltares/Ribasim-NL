...
def get_dhydro_files(simulation_path: Path):
    ...
    input_files["external_forcing_file"] = Path(mdu_dir, mdu["external forcing"]["extforcefilenew"])
...
def get_dhydro_branches_from_network_data(network_data, crs):
    ...
    branches_gdf = gpd.GeoDataFrame(branches_df, geometry="geometry", crs=crs).drop("branch_geom", axis=1)
...
def check_number_of_pumps_at_pumping_station(pumps_gdf: gpd.GeoDataFrame, set_name: str):
    ...
    pumps_gdf = gpd.GeoDataFrame(pumps_gdf.drop(columns=("geometry", "")), geometry=pumps_gdf["geometry"], crs=crs)
    return pumps_gdf
...
def split_dhydro_structures(structures_gdf: gpd.GeoDataFrame, set_name: str):
    ...
    for structure_type in list_structure_types:
        ...
        structure_gdf = gpd.GeoDataFrame(
            pd.concat(
                [structure_gdf[header_0], structure_gdf[header_1], structure_gdf[header_2]],
                keys=["general", "structure", set_name],
                axis=1,
            ),
...
def get_dhydro_external_forcing_locations(
    ...
    boundaries_gdf = network_nodes_gdf[["network_node_id", "geometry"]].merge(
        boundaries_gdf, how="right", left_on="network_node_id", right_on="network_node_id"
    )
    ...
    boundaries_gdf = boundaries_gdf.rename(columns={"name": "boundary_name", "quantity": "boundary_type"}).drop(
        columns="index_right"
    )
...
def get_dhydro_forcing_data(mdu_input_dir: Path, boundaries_gdf: gpd.GeoDataFrame, laterals_gdf: gpd.GeoDataFrame):
...
def get_dhydro_volume_based_on_basis_simulations(
    mdu_input_dir: Path, volume_tool_bat_file: Path, volume_tool_force: bool = False, volume_tool_increment: float = 0.1
):
...
def get_dhydro_data_from_simulation(
    ...
    boundaries_data, laterals_data = get_dhydro_forcing_data(
        mdu_input_dir=mdu_input_dir, boundaries_gdf=boundaries_gdf, laterals_gdf=laterals_gdf
    )
