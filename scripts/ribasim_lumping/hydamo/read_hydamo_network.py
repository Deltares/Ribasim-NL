...
def add_hydamo_basis_network(
    ...
    pumps_gdf = read_geom_file(filepath=hydamo_network_file, layer_name="gemaal", crs=crs)
    print(f" pumps ({len(pumps_gdf) if pumps_gdf is not None else 0}x)", end=", ")

    sluices_gdf = read_geom_file(filepath=hydamo_network_file, layer_name="sluis", crs=crs)
    print(f"sluices ({len(sluices_gdf) if sluices_gdf is not None else 0}x)", end=", ")

    weirs_gdf = read_geom_file(filepath=hydamo_network_file, layer_name="stuw", crs=crs)
    print(f"weirs ({len(weirs_gdf) if weirs_gdf is not None else 0}x)", end=", ")

    culverts_gdf = read_geom_file(filepath=hydamo_network_file, layer_name="duikersifonhevel", crs=crs)
    print(f"culverts ({len(culverts_gdf) if culverts_gdf is not None else 0}x)", end=", ")

    closers_gdf = read_geom_file(filepath=hydamo_network_file, layer_name="afsluitmiddel", crs=crs)
