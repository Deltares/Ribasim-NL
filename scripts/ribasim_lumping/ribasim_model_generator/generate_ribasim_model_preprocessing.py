...
def get_waterlevels_table_from_simulations(map_data):
    ...
    node_h_df = pd.concat([node_h_df1[col].sort_values().reset_index(drop=True) for col in node_h_df1.columns], axis=1)
...
def get_basins_outflows_including_settings(
    split_nodes, basin_connections, boundary_connections, weirs, uniweirs, pumps, culverts, orifices, set_names
):
    gdfs_list = [weirs, uniweirs, pumps, culverts, orifices]
    gdfs_columns_list = [
        [("general", "structure_id"), ("structure", "crestwidth")]
        + [(set_name, "crestlevel") for set_name in set_names],
        [("general", "structure_id")] + [(set_name, "crestlevel") for set_name in set_names],
        [
            ("general", "structure_id"),
            ("structure", "orientation"),
            ("structure", "controlside"),
            ("structure", "numstages"),
            ("structure", "capacity"),
        ]
    ...
    basins_outflows1 = basins_outflows1[["basin", "split_node"]].merge(basins_split_nodes, how="left", on="split_node")
    ...
    basins_outflows = basins_outflows.merge(
        gdf_total, how="left", left_on="general__split_node_id", right_on="general__structure_id"
    )
    basins_outflows = (
        basins_outflows.sort_values(by="general__basin").reset_index(drop=True).drop(columns=["general__structure_id"])
    )
    ...
    for set_name in set_names:
        ...
        if set_name + "__crestlevel" in basins_outflows.columns:
            basins_outflows[set_name + "__targetlevel"] = basins_outflows[set_name + "__targetlevel"].fillna(
                basins_outflows[set_name + "__crestlevel"]
            )
        if set_name + "__stoplevelsuctionside" in basins_outflows.columns:
            basins_outflows[set_name + "__targetlevel"] = basins_outflows[set_name + "__targetlevel"].fillna(
                basins_outflows[set_name + "__stoplevelsuctionside"]
            )
        basins_outflows["general__split_node_node_id"] = (
            basins_outflows["general__split_node_node_id"].fillna(-1).astype(int)
        )
...
def get_targetlevels_nodes_using_weirs_pumps(nodes, basins_outflows, set_names, name_column="targetlevel"):
    ...
    for set_name in set_names:
        basins_outflows_sel[set_name] = (
            basins_outflows_sel[set_name].apply(lambda x: convert_list_str_to_float(x)).astype(float)
        )
...
def generate_node_waterlevels_table(node_h_df, node_bedlevel, node_targetlevel, interpolation_lines, set_names):
    ...
    for set_name in set_names:
        ...
        if node_h_df is None:
            node_h_set = pd.concat([node_basis.copy()], keys=[set_name], names=["set"])
            node_h = pd.concat([node_h, node_h_set])
        else:
            node_h_set = pd.concat(
                [pd.concat([node_basis, node_h_df.loc[set_name]])], keys=[set_name], names=["set"]
            ).interpolate(axis=0)
            node_h = pd.concat([node_h, node_h_set])
...
def translate_waterlevels_to_surface_storage(nodes_h, curve, orig_bedlevel, decimals=3):
    ...
    for col in nodes_x.columns:
        ...
        nodes_h_new.loc[nodes_x[col] < level_correction_bedlevel, col] = (
            math.floor(level_correction_bedlevel * 10) / 10.0
        )
        interp_func = scipy.interpolate.interp1d(
            curve.index, curve[col].to_numpy(), kind="linear", fill_value="extrapolate"
        )
...
def generate_surface_storage_for_basins(node_a, node_v, nodes):
    ...
    for set_name in node_a.index.get_level_values(0).unique():
        basin_a_set = (
            node_a.loc[set_name]
            .T.merge(nodes, how="inner", left_index=True, right_on="node_no")
            .drop(columns=["node_no"])
        )
...
def generate_h_relation_basins_nodes(nodes, node_h_basin, basin_h):
    ...
    node_h_results = node_h_basin_stacked.reset_index().merge(nodes_basin, how="left", on="node_no")
    ...
    basin_node_h_relation = gpd.GeoDataFrame(
        node_h_results.merge(basin_h_results, how="left", on=("set", "condition", "basin_node_id")),
        geometry="geometry",
        crs=nodes.crs,
    )
...
def preprocessing_ribasim_model_tables(
    ...
    node_surface_df, node_storage_df, node_bedlevel, orig_bedlevel = extract_bed_level_surface_storage(
        volume_data, nodes
    )
    node_h_df = get_waterlevels_table_from_simulations(map_data)
    node_h_basin = generate_node_waterlevels_table(
        node_h_df, node_bedlevel, node_targetlevel, interpolation_lines, set_names
    )
    node_h_node, node_a, node_v = generate_surface_storage_for_nodes(
        node_h_basin, node_surface_df, node_storage_df, orig_bedlevel, set_names
    )
    ...
    basins_nodes_h_relation = generate_h_relation_basins_nodes(nodes=nodes, node_h_basin=node_h_basin, basin_h=basin_h)
