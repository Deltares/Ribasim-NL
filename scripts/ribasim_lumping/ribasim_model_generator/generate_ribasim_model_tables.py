...
def generate_basin_profile_table(
    basin_h: pd.DataFrame, basin_a: pd.DataFrame, basins: pd.DataFrame, set_name: int, decimals=3, dummy_model=False
):
    ...
    basin_profile = basin_profile.sort_values(by=["node_id", "level", "area"]).drop_duplicates(
        subset=["node_id", "level"], keep="first"
    )
    ...
    new_basin_profile = new_basin_profile.sort_values(by=["node_id", "level", "area"]).reset_index(drop=True)
...
def generate_basin_time_table_laterals(basins, basin_areas, laterals, laterals_data, saveat):
    laterals_basins = (
        ...
        .sjoin(basin_areas[["basin_node_id", "geometry"]])
        .drop(columns=["index_right"])[["id", "basin_node_id"]]
    )
    ...
    for basin_node_id in basin_node_ids:
        ...
        timeseries = pd.concat([timeseries, timeseries_basin])
    timeseries = timeseries.sort_values(["time", "node_id"]).reset_index(drop=True)
    timeseries = timeseries[
        ["time", "node_id", "precipitation", "potential_evaporation", "drainage", "infiltration", "urban_runoff"]
    ]
...
def generate_basin_time_table_laterals_areas_data(basins, areas, laterals_areas_data):
    ...
    for basin_node_id in basin_node_ids:
        ...
        timeseries_basin = (
            laterals_areas_data[areas_basin].sum(axis=1).to_frame().rename(columns={0: "Netto_flux"}).reset_index()
        )
...
def generate_basin_time_table(
    ...
    if dummy_model:
        ...
        basin_time = generate_basin_time_table_laterals(
            basins=basins, basin_areas=basin_areas, laterals=laterals, laterals_data=laterals_data, saveat=saveat
        )
        ...
        basin_time = generate_basin_time_table_laterals_areas_data(
            basins=basins, areas=areas, laterals_areas_data=laterals_areas_data
        )
        ...
        basin_time = generate_basin_time_table_laterals_drainage_per_ha(
            basins=basins, basin_areas=basin_areas, laterals_drainage_per_ha=laterals_drainage_per_ha
        )
...
def generate_tabulated_rating_curve(
    ...
    basins_outflows_sel = basins_outflows_sel[
        ["general__split_node", "structure__crestwidth", set_name + "__crestlevel"]
    ]
    ...
    for i, trc in tabulated_rating_curves.iterrows():
        if dummy_model:
            curve = pd.DataFrame(
                dict(node_id=trc["split_node_node_id"], level=[0.0, 1.0, 1.1], flow_rate=[0.0, 0.0, 1.0])
            )
        ...
        if discharges is None:
            ...
            curve["flow_rate"] = curve.apply(
                lambda x: weir_formula(trc.crestlevel, trc.crestwidth, x["level"]), axis=1
            ).fillna(0.0)
        ...
        if no_negative_total > 0:
            print(
                f" x {trc.split_node_id} ({split_type}): {no_negative_total}/{no_total} records with decreasing discharge"
            )
...
def generate_boundary_time_table(boundaries, boundaries_data, boundary_type):
    ...
    for i_boundary, boundary in boundaries.iterrows():
        ...
        timeseries = pd.concat([timeseries, timeseries_boundary])
    timeseries = timeseries.sort_values(["time", "node_id"]).reset_index(drop=True)
...
def generate_boundary_static_data(boundaries, boundaries_static_data):
    ...
    for boundary in boundaries:
        ...
        boundary_data_boundary = (
            boundaries_static_data[boundary_no].sum(axis=1).to_frame().rename(columns={0: "flow"}).reset_index()
        )
        boundary_data_boundary["flow"] = boundary_data_boundary["flow"].fillna(dummyvalue)
        boundary_data_boundary["node_id"] = boundary_no

        boundary_data = pd.concat([boundary_data, boundary_data_boundary])
...
def generate_basin_state_table(basin_h_initial, basin_profile, basin_h, set_name=None, dummy_model=False):
    ...
    if basin_h_initial is None:
        basin_state = (
            basin_h.loc[(set_name, "targetlevel")]
            .rename("level")
            .reset_index()
...
def generate_basin_subgrid_table(basins_nodes_h_relation, set_name, dummy_model=False, basins=None):
    ...
    basin_subgrid = basins_nodes_h_relation[
        (basins_nodes_h_relation["set"] == set_name) & (basins_nodes_h_relation["basin_node_id"] != -1)
    ]
    ...
    basin_subgrid = basin_subgrid.rename(
        columns={
            "node_no": "subgrid_id",
    ...
    for i in range(3):
        ...
        for node_id in basin_subgrid.node_id.unique():
            basin_subgrid.loc[
                (basin_subgrid["node_id"] == node_id).idxmin(), ["basin_level_diff", "subgrid_level_diff"]
            ] = np.nan
...
def generate_ribasim_model_tables(
    dummy_model,
    basin_h,
    basin_a,
    basins,
    ...
    tables["basin_profile"] = generate_basin_profile_table(
        basin_h=basin_h, basin_a=basin_a, basins=basins, decimals=3, set_name=set_name, dummy_model=dummy_model
    )
    ...
    tables["basin_subgrid"] = generate_basin_subgrid_table(
        basins_nodes_h_relation=basins_nodes_h_relation, set_name=set_name, dummy_model=dummy_model, basins=basins
    )
    ...
    if dummy_model:
        tables["flow_boundary_time"] = (
            pd.concat(
                [
                    pd.DataFrame(dict(time=pd.date_range(start_time, end_time), node_id=node_id, flow_rate=1.0))
        ...
        tables["level_boundary_time"] = (
            pd.concat(
                [
                    pd.DataFrame(dict(time=pd.date_range(start_time, end_time), node_id=node_id, level=0.0))
