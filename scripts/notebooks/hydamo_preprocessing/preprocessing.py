...
def add_wfd_id_to_hydroobjects(hydroobjects: gpd.GeoDataFrame, 
    ...
    hydroobjects['left_polygon_geometry'] = hydroobjects.geometry.buffer(
        buffer_distance, join_style="round")
    
    wfd_total = None
    if wfd_lines is not None:
        wfd_lines["geometry"] = wfd_lines.geometry.buffer(
            buffer_distance*1.5, join_style="round")
    ...
    hydroobjects = get_most_overlapping_polygon(hydroobjects, 
        wfd_total, 'code', wfd_id_column)
    
    # 5. Remove overlapping polygons with less than 90 % overlap
    hydroobjects[wfd_id_column] = hydroobjects.apply(lambda x:
        x["most_overlapping_polygon_id"] if x["most_overlapping_polygon_area"] > overlap_ratio * x['left_area'] else None
        if type(x["most_overlapping_polygon_area"]) == float else None, axis = 1) 
    ...
    hydroobjects = hydroobjects.drop(
        columns = ['left_area','left_polygon_geometry',
                   'overlapping_areas','line_geometry',
                   'most_overlapping_polygon_id',
                   'most_overlapping_polygon_area']) #'overlapping_area' 'left_area'
...
def preprocess_hydamo_hydroobjects(hydroobjects: gpd.GeoDataFrame,
                                   wfd_lines: gpd.GeoDataFrame = None,
                                   wfd_polygons: gpd.GeoDataFrame = None,
                                   buffer_distance_endpoints: float = 0.5,
                                   wfd_id_column: str = 'owmident',
                                   buffer_distance_wfd: float = 10,
                                   overlap_ratio_wfd: float = 0.9) -> gpd.GeoDataFrame:
    """
