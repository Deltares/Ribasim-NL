...
def run_pipeline(config: PipelineConfig, *, dry_run: bool) -> None:
    ...
    for script in config.full_control_scripts:
        run_step(f"Full control: {script.parent.name}", [*python_command, str(script)], env, log_dir, dry_run=dry_run)
...
def main() -> int:
    ...
    for key in selected_keys:
        run_pipeline(PIPELINES[key], dry_run=args.dry_run)
    return 0
