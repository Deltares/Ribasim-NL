...
## Uitlaat 988 is de hoofdroute voor de vollopende Zoom/Schelde-Rijnkanaal-keten.
## Tijdelijk als pomp modelleren, omdat afwatering anders vastloopt op hoge RWS-waterstanden.
temporary_schelde_rijnkanaal_pump_node_id = 988
model.update_node(node_id=temporary_schelde_rijnkanaal_pump_node_id, node_type="Pump")
model.node.df.loc[temporary_schelde_rijnkanaal_pump_node_id, "name"] = (
    "Schelde-Rijnkanaal tijdelijke pomp naar RWS (eigenlijk uitlaat)"
)
...
for static_df in (model.outlet.static.df, model.pump.static.df):
    lower_aanvoer_max_downstream_level(static_df, AANVOER_MAX_DOWNSTREAM_MARGIN)
...
pump_max_flow_rate_from_results = {
    ...
    988: 19.8,  # Tijdelijke pomp naar Schelde-Rijnkanaal; behoud de oude afvoercapaciteit van de uitlaat.
}
mask = (
...
manual_max_flow_rate_node_ids = {376, 446, 509, 988, 1056}
