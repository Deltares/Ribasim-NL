...
for row in basin_df.itertuples():
    if row.Index not in ignore_basins:
        links_mask = (model.link.df.from_node_id == row.Index) | (model.link.df.to_node_id == row.Index)
        links_geom = model.link.df[links_mask].union_all()

        selected_areas = dissolved_area_gdf[dissolved_area_gdf.intersects(links_geom)]
