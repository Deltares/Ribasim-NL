...
for toml_path in toml_paths:
    print(toml_path.parent)
    if toml_path.parent.joinpath(r"results/Eindbeoordeling_model.xlsx").exists():
        continue
    loc_koppeltabel = toml_path.parent.joinpath(f"{source_excel_path.stem}_{toml_path.stem}.xlsx")
    ########################################################################################################################################
    ...
    loc_specifieke_bewerking = toml_path.parent.joinpath(
        f"Specifiek_bewerking_versieSamenwerkdag_26052026_{toml_path.stem}.xlsx"
    )

    meas_folder = cloud.joinpath("Basisgegevens/resultaatvergelijking/meetreeksen_2026")

    model_folder = toml_path.parent
    toml_naam = toml_path.name
    # model_folder = Path(
    #     r"d:/repositories/Ribasim-NL/data/Rijkswaterstaat/modellen/lhm_sub_models/AAM-Limburg-RWS_coupled"
    # )
