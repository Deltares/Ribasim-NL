...
def split_linestring_by_indices(linestring: LineString, split_indices: list) -> list:
    ...
    """
    split_linestrings = []
    split_indices = sorted(list(set([0] + split_indices + [len(linestring.coords) - 1])))  # NOQA FIXME: list() weghalen
...
def remove_duplicate_split_lines(lines: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
    Removes duplicates of split lines from line feature vector dataset. Duplicates are removed from a subselection from the line feature dataset that contains all line features that have been split.
    ...
    """
    lines["distance"] = list(map(lambda x: x.length, lines["geometry"]))  # NOQA FIXME: Daniel Tollenaar, volgens mij kun je hier gewoon lines["distance"] = lines.length van maken
    ...
    for split_action in splits:
        ...
        for node in split_action["split_points"]:
            ...
            if index_nearest_neighbour == 0 and linestring.coords[0] in list(
                connections_to_create.loc[connections_to_create["inserted"] == True, "point"].values  # NOQA FIXME: == True kan weg
            ):
                continue
...
def connect_endpoints_by_buffer(lines: gpd.GeoDataFrame, buffer_distance: float = 0.5) -> gpd.GeoDataFrame:
    ...
    while not finished:
        ...
        boundary_endpoints["overlaying_line_buffers"] = list(  # noqa: C417: list-comprenension will do
        ...
        boundary_endpoints["startpoint_overlaying_line_buffers"] = boundary_endpoints.apply(
            lambda x: list(  # noqa: C417: list-comprenension will do
                map(
        ...
        boundary_endpoints["endpoint_overlaying_line_buffers"] = boundary_endpoints.apply(
            lambda x: list(  # noqa: C417: C417: list-comprenension will do
                map(
        ...
        unconnected_endpoints = boundary_endpoints[
            boundary_endpoints["crossed_by_unconnected_lines"] == True  # noqa: E712: == True kan weg
        ].reset_index(drop=True)
...
def get_most_adjacent_polygon_within_gdf(left_gdf, left_id, right_gdf=None, right_id=None):
    def get_most_intersecting(gdf, polygon, left_id):
        ...
        except:  # noqa: E722 FIXME: expliciet de juiste exception afvangen
            return None

    left_gdf = get_touching_polygons_from_within_gdf(left_gdf, left_id)
    if type(right_gdf) == gpd.GeoDataFrame:  # noqa: E721: FIXME: isinstance(right_gdf, gpd.GeoDataFrame)
    ...
    left_gdf["touching_polygons"] = left_gdf["touching_polygons"].apply(lambda x: x[x["basin"] != None])  # noqa: E711 FIXME: is not None
    left_gdf["touching_polygons"] = left_gdf["touching_polygons"].apply(lambda x: x[x["basin"].isna() == False])  # noqa: E712: FIXME: vervang met not x["basin"].isna()
    if type(right_gdf) == gpd.GeoDataFrame:  # noqa: E721: FIXME: isinstance(right_gdf, gpd.GeoDataFrame). waarschijnlijk moet right_gdf hier left_gdf zijn
        left_gdf["touching_polygons"] = left_gdf.apply(
            lambda x: x["touching_polygons"][x["touching_polygons"]["right_id"] == x["right_id"]]
            if x["right_id"] != None  # noqa: E711 FIXME is not None
